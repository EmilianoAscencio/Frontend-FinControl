      ...
      <template v-else>
        ...
        <section class="section">
          <h3 class="section-title">
            ...
            <span class="count">{{ store.ingresos.length }}</span>
          </h3>
          <div v-if="store.ingresos.length === 0" class="empty">Sin categorías de ingreso.</div>
          <div class="cat-grid">
            <div
              v-for="cat in store.ingresos"
              :key="cat.id"
              class="cat-card"
              :style="cat.color ? `border-left: 4px solid ${cat.color}` : ''"
            >
              <div class="cat-icon" :style="cat.color ? `background:${cat.color}22` : ''">
                {{ cat.icon || '💰' }}
              </div>
              <div class="cat-info">
...
.cat-icon {
  ...
  font-size: 18px; background: var(--accent-bg); flex-shrink: 0;
}
...
.close-btn {
  ...
  font-size: 16px; color: var(--text); padding: 4px;
}
...
.icon-btn {
  ...
  font-size: 18px; cursor: pointer; transition: border-color 0.15s, background 0.15s;
}
